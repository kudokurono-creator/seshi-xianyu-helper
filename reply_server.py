...
app.include_router(
    create_local_llm_router(
        ...
        get_setting=db_manager.get_system_setting,
        set_setting=db_manager.set_system_setting,
    )
)
...
class AIModelsRequest(BaseModel):
    """Credentials used to discover models from an OpenAI-compatible API."""
    base_url: str
    api_key: str
...
async def discover_ai_models(
    ...
    """Discover available model IDs from the configured provider."""
    ...
    if not models_url.startswith(("https://", "http://")):
    ...
    try:
        ...
        logger.warning(f"获取AI模型列表失败（网络错误）: {type(exc).__name__}")
        raise HTTPException(status_code=502, detail="无法连接 AI API，请检查地址和网络")

    if response.status_code >= 400:
        logger.warning(f"获取AI模型列表失败: HTTP {response.status_code}")
        raise HTTPException(status_code=502, detail=f"AI API 返回 HTTP {response.status_code}")
    ...
    if not models:
        raise HTTPException(status_code=502, detail="AI API 未返回可用模型列表")
...
# 手动添加商品路由必须放在上述固定路径之后，避免将 get-all-from-account 解析为 cookie_id。
...
def create_item(cookie_id: str, item_data: ItemCreateRequest, current_user: Dict[str, Any] = Depends(get_current_user)):
    """手动添加商品到指定账号。"""
    try:
        if cookie_id not in db_manager.get_all_cookies(current_user['user_id']):
            raise HTTPException(status_code=403, detail="无权限操作该Cookie")
        item_id, item_title = item_data.item_id.strip(), item_data.item_title.strip()
        if not item_id or not item_title:
            raise HTTPException(status_code=422, detail="商品ID和商品标题不能为空")
        detail = json.dumps({
        ...
        if not db_manager.save_item_basic_info(cookie_id, item_id, item_title, item_price=item_data.item_price or "", item_detail=detail):
            raise HTTPException(status_code=500, detail="保存商品失败")
        db_manager.update_item_multi_spec_status(cookie_id, item_id, item_data.is_multi_spec)
        db_manager.update_item_multi_quantity_delivery_status(cookie_id, item_id, item_data.multi_quantity_delivery)
        item = db_manager.get_item_info(cookie_id, item_id) or {}
        item["item_image"] = item_data.item_image or ""
        item["is_multi_qty_ship"] = item_data.multi_quantity_delivery
        return {"success": True, "message": "商品添加成功", "item": item}
    except HTTPException:
        raise
    except Exception as e:
