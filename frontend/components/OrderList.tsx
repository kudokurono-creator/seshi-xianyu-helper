...
const StatusBadge: React.FC<{ status: OrderStatus }> = ({ status }) => {
  const styles = {
    ...
    pending_ship: 'bg-spectral-accent/15 text-spectral-accent-bright',
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
        <div>
          <h2 className="text-4xl font-bold text-gray-900 tracking-tight">订单中心</h2>
          <p className="text-gray-500 mt-2 font-medium">查看所有 seshi 交易记录与状态。</p>
        </div>
        <div className="flex items-center gap-3">
            <button onClick={loadOrders} className="p-3 rounded-2xl ios-btn-secondary text-gray-600 transition-colors">
            ...
            <button onClick={(event) => void runSpectralAction(event.currentTarget, handleSync, { action: 'sync-orders' })} className="ios-btn-primary px-6 py-3 rounded-2xl font-bold text-sm flex items-center gap-2">
      ...
      <div className="ios-card rounded-[2rem] overflow-hidden border-0">
        {/* Toolbar */}
        <div className="p-4 border-b border-gray-50 flex flex-col md:flex-row gap-4 justify-between items-center bg-spectral-primary">
          <div className="flex gap-1 p-1 bg-spectral-secondary rounded-xl overflow-x-auto max-w-full">
             {[
                 {k:'all', v:'全部'},
                 {k:'shipped', v:'已发货'},
                 ...
                 <button
                    ...
                    className={`px-5 py-2 rounded-lg text-sm font-bold transition whitespace-nowrap ${filter === opt.k ? 'bg-spectral-accent/10 text-spectral-accent-bright' : 'text-gray-500 hover:text-gray-700 hover:bg-spectral-hover'}`}
                 >
                    {opt.v}
                 </button>
             ))}
          ...
          <div className="relative w-full md:w-auto group">
             <Search className="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-spectral-accent-bright transition-colors" />
             <input
                 type="text"
                 placeholder="搜索订单号/商品/买家..."
                 value={searchText}
                 onChange={(e) => { setSearchText(e.target.value); setPage(1); }}
                 className="ios-input pl-10 pr-4 py-2.5 rounded-xl w-64 border-none focus:ring-0"
             />
          </div>
        </div>
        ...
        <div className="overflow-x-auto min-h-[240px]">
          {loading ? (
            <SpectralState state="loading" type="orders" title="正在加载订单…" description="正在读取当前筛选条件下的交易记录" compact />
          ) : orders.length === 0 ? (
            <SpectralState state="empty" type="orders" title="暂无订单" description="同步订单后，交易记录会显示在这里" compact />
          ) : (
            <table className="w-full text-left border-collapse table-fixed">
            <thead>
              <tr className="bg-spectral-secondary text-gray-400 text-xs font-bold uppercase tracking-wider border-b border-gray-50">
            ...
            <tbody className="divide-y divide-gray-50">
              {orders.map((order) => (
                <tr key={order.id} className="hover:bg-spectral-hover transition-colors group">
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-5">
                      <div className="w-14 h-14 rounded-xl bg-gray-100 overflow-hidden shadow-sm border border-gray-100 flex-shrink-0">
                        {order.item_image ? (
                            <img src={order.item_image} alt={`${getItemNameById(order.item_id, order.item_title)}商品图`} width="56" height="56" loading="lazy" className="w-full h-full object-cover" />
                        ) : (
                  ...
                  <td className="px-6 py-5 text-right">
                    {order.status === 'pending_ship' && (
                        <button
                            onClick={(event) => void runSpectralAction(event.currentTarget, () => handleShip(order.order_id), { action: 'ship-order', immediate: true })}
                            className="mr-2 ios-btn-primary text-xs font-bold px-3 py-2 rounded-xl transition active:scale-95"
                        >
                            立即发货
                        </button>
                    )}
        ...
        {/* Pagination */}
        {!loading && orders.length > 0 && <div className="p-4 border-t border-gray-50 flex items-center justify-between bg-spectral-primary">
      ...
      {showDetailModal && selectedOrder && createPortal(
        <div className="modal-overlay-centered">
          <div role="dialog" aria-modal="true" aria-label="订单详情" className="modal-container">
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-gray-50 rounded-xl flex items-center gap-4">
                  {selectedOrder.item_image && (
                    <img src={selectedOrder.item_image} alt={`${getItemNameById(selectedOrder.item_id, selectedOrder.item_title)}商品图`} width="80" height="80" className="w-20 h-20 rounded-xl object-cover border border-gray-200" />
                  )}
            ...
            <div className="modal-footer">
              <div className="flex gap-3 w-full">
                ...
                {selectedOrder.status === 'pending_ship' && (
                  <button
                    onClick={(event) => void runSpectralAction(event.currentTarget, () => {
                      setShowDetailModal(false);
                      handleShip(selectedOrder.order_id);
                    }, { action: 'ship-order', immediate: true })}
                    className="flex-1 px-6 py-3 rounded-xl ios-btn-primary font-bold"
                  >
                    立即发货
                  </button>
                )}
      ...
      {showImportModal && createPortal(
        <div className="modal-overlay-centered">
          <div role="dialog" aria-modal="true" aria-label="重发订单" className="modal-container">
            <div className="modal-header">
              <div className="flex items-center justify-between w-full">
            ...
            <div className="modal-footer">
              <div className="flex gap-3 w-full">
                ...
                <button
                  onClick={(event) => void runSpectralAction(event.currentTarget, handleImportOrders, { action: 'import-orders' })}
                  disabled={!importFile}
                  className="flex-1 px-6 py-3 rounded-xl ios-btn-primary font-bold disabled:opacity-50"
                >
                  导入订单
                </button>
              </div>
      ...
      {showShipModal && createPortal(
        <div className="modal-overlay-centered">
          <div role="dialog" aria-modal="true" aria-label="订单操作" className="modal-container" style={{ maxWidth: '480px' }}>
            <div className="modal-header">
              <div className="flex items-center justify-between w-full">
            ...
            <div className="modal-body space-y-4">
              ...
              {/* 选项A: 仅修改发货状态 */}
              <button
                onClick={(event) => void runSpectralAction(event.currentTarget, () => executeShip('status_only'), { action: 'ship-order' })}
                disabled={shipLoading}
                className="w-full text-left p-4 rounded-xl border-2 border-gray-200 hover:border-gray-400 hover:bg-gray-50 transition disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="font-bold text-gray-900 text-sm">仅修改 seshi 发货状态</div>
                    <div className="text-xs text-gray-500 mt-1 leading-relaxed">
                      不实际扣除或发送卡券，仅在 seshi 平台将订单标记为"已发货"。
                      适用于已经给客户发过货、只是忘记在 seshi 修改状态的情况。
                    </div>
                  </div>
                </div>
              </button>
              ...
              {/* 选项B: 完整发货流程 */}
              <button
                onClick={(event) => void runSpectralAction(event.currentTarget, () => executeShip('full_delivery'), { action: 'ship-order' })}
                disabled={shipLoading}
                className="w-full text-left p-4 rounded-xl border-2 border-gray-200 hover:border-spectral-accent/30 hover:bg-spectral-accent/5 transition disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-xl bg-spectral-accent/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                    <PackageCheck className="w-5 h-5 text-spectral-accent-bright" />
                  </div>
                  <div>
      ...
      {showEditModal && editingOrder && createPortal(
        <div className="modal-overlay-centered">
          <div role="dialog" aria-modal="true" aria-label="编辑订单" className="modal-container">
            <div className="modal-header">
              <div className="flex items-center justify-between w-full">
