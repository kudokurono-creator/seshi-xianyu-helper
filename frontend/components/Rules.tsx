  ...
  return (
    <div className="space-y-6 animate-fade-in pb-20">
      <div className="flex justify-between items-center">
        ...
        <button onClick={refresh} className="p-3 ios-btn-secondary rounded-xl transition-shadow">
      ...
      <div className="flex p-1.5 bg-gray-200/50 rounded-2xl w-fit">
          <button 
            ...
            className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition ${activeTab === 'shipping' ? 'bg-spectral-accent/10 text-spectral-accent-bright' : 'text-gray-500 hover:text-gray-700'}`}
          >
              <Zap className="w-4 h-4" /> 自动发货规则
          </button>
          <button 
            onClick={() => setActiveTab('reply')}
            className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition ${activeTab === 'reply' ? 'bg-spectral-accent/10 text-spectral-accent-bright' : 'text-gray-500 hover:text-gray-700'}`}
          >
          ...
          {activeTab === 'shipping' && (
              <div className="space-y-4">
                  <div className="flex justify-between items-center mb-6">
                          <div className="flex items-center gap-2 text-sm text-status-warning bg-status-warning/10 px-4 py-2 rounded-xl">
                              <AlertCircle className="w-4 h-4" />
                              付款成功后自动发送对应卡密；文本型卡密（如网盘链接）会对每个订单重复发送。
                          </div>
                      <button onClick={handleAddShippingRule} className="ios-btn-primary px-5 py-3 rounded-2xl text-sm font-bold flex items-center gap-2">
                  ...
                  <div className="space-y-3">
                      {shippingRules.map(rule => (
                          <div key={rule.id} className="flex items-center justify-between p-5 rounded-2xl border border-gray-100 bg-[#F7F8FA] hover:bg-white hover:shadow-lg transition duration-300">
                              <div className="flex items-center gap-4">
                                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-lg ${rule.enabled ? 'bg-spectral-secondary text-spectral-accent-bright' : 'bg-gray-200 text-gray-400'}`}>
                                      {rule.priority}
                                  </div>
                                  <div>
                              ...
                              <div className="flex items-center gap-3">
                                  ...
                                  <button
                                    type="button"
                                    aria-label={rule.enabled ? '停用发货规则' : '启用发货规则'}
                                    role="switch"
              ...
              <div className="space-y-4">
                  <div className="flex justify-between items-center mb-6">
                      ...
                      <button
                        ...
                        className="ios-btn-primary px-5 py-3 rounded-2xl text-sm font-bold flex items-center gap-2"
                      >
                  ...
                  <div className="space-y-3">
                      {replyRules.map(rule => (
                          <div key={rule.id} className="flex flex-col md:flex-row md:items-center justify-between p-5 rounded-2xl border border-gray-100 bg-[#F7F8FA] hover:bg-white hover:shadow-lg transition duration-300 gap-4">
                              ...
                              <div className="flex items-center gap-4 border-t md:border-t-0 md:border-l border-gray-200 pt-4 md:pt-0 md:pl-6">
                                  ...
                                  <button
                                    type="button"
                                    aria-label={rule.enabled ? '停用回复规则' : '启用回复规则'}
                                    role="switch"
                                    aria-checked={rule.enabled}
      ...
      {showShippingModal && (
        <div className="modal-overlay">
          <div role="dialog" aria-modal="true" aria-label="发货规则" className="modal-container">
            ...
            <div className="modal-body space-y-5">
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                ...
                <button
                  ...
                  role="switch"
                  aria-label="启用发货规则"
                  aria-checked={Boolean(editingShippingRule?.enabled)}
                  ...
                  className={`w-14 h-8 rounded-full transition-colors duration-300 relative ${
                    editingShippingRule?.enabled ? 'bg-spectral-accent/35' : 'bg-gray-300'
                  }`}
                >
                  <span
      ...
      {showReplyModal && (
        <div className="modal-overlay">
          <div role="dialog" aria-modal="true" aria-label="回复规则" className="modal-container">
            <div className="modal-header">
              <div className="flex items-center justify-between w-full">
                <h3 className="text-2xl font-extrabold text-gray-900">
            ...
            <div className="modal-body space-y-5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`p-3 rounded-xl font-bold transition ${
                      editingReplyRule?.match_type === 'exact' ? 'bg-spectral-accent/15 text-spectral-accent-bright' : 'bg-gray-100 text-gray-600'
                    }`}
                  >
                    精确匹配
                  </button>
                  <button
                    ...
                    className={`p-3 rounded-xl font-bold transition ${
                      editingReplyRule?.match_type === 'fuzzy' ? 'bg-spectral-accent/15 text-spectral-accent-bright' : 'bg-gray-100 text-gray-600'
                    }`}
                  >
                    模糊包含
                  </button>
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                ...
                <button
                  ...
                  role="switch"
                  aria-label="启用回复规则"
                  aria-checked={Boolean(editingReplyRule?.enabled)}
                  onClick={() => setEditingReplyRule({ ...editingReplyRule, enabled: !editingReplyRule?.enabled })}
                  className={`w-14 h-8 rounded-full transition-colors duration-300 relative ${
                    editingReplyRule?.enabled ? 'bg-spectral-accent/35' : 'bg-gray-300'
                  }`}
                >
                  <span
                    className={`absolute top-1 w-6 h-6 bg-white rounded-full shadow-md transition-transform duration-300 block ${
