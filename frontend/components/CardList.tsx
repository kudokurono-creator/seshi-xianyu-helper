...
const CardList: React.FC = () => {
  ...
  const handleAddCard = async () => {
    try {
      // 添加弹窗使用统一的 content 字段，提交前转换为后端实际字段。
      // 否则文本卡密（如网盘链接）会被后端当成未知字段而丢弃。
      const payload: any = {
        name: addForm.name,
        type: addForm.type,
      ...
      if (addForm.type === 'text') {
        payload.text_content = addForm.content;
      } else if (addForm.type === 'image') {
        payload.image_url = addForm.content;
      } else if (addForm.type === 'api') {
        payload.api_config = { url: addForm.content, method: 'GET', timeout: 10 };
      }
  ...
  const toggleCardStatus = async (card: Card) => {
    try {
      await updateCard(String(card.id), { ...card, enabled: !card.enabled });
      getCards().then(setCards);
    } catch (error) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center">
        ...
        <button
            onClick={(event) => void runSpectralAction(event.currentTarget, () => setShowAddModal(true), { action: 'add-card', immediate: true })}
                  ...
                  <tr key={card.id} className="hover:bg-[#FFFDE7]/50 transition-colors group">
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-spectral-secondary rounded-xl group-hover:bg-spectral-hover transition-colors">
      ...
      {showEditModal && selectedCard && createPortal(
        <div className="modal-overlay-centered">
          <div role="dialog" aria-modal="true" aria-label="编辑卡密组" className="modal-container">
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                  ...
                  <button
                    ...
                    className={`w-14 h-8 rounded-full transition-colors duration-300 relative ${
                      editForm.enabled ? 'bg-spectral-accent/35' : 'bg-gray-300'
                    }`}
                  >
                    <span
            ...
            <div className="modal-footer">
              <div className="flex gap-3 w-full">
                ...
                <button
                  onClick={(event) => void runSpectralAction(event.currentTarget, handleSaveEdit, { action: 'save-card' })}
      ...
      {showAddModal && createPortal(
        <div className="modal-overlay-centered">
          <div role="dialog" aria-modal="true" aria-label="新增卡密组" className="modal-container">
            <div className="modal-header">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <button
                      ...
                      className={`p-3 rounded-xl font-bold transition ${addForm.type === 'text' ? 'bg-spectral-accent/15 text-spectral-accent-bright' : 'bg-gray-100 text-gray-600'}`}
                    >
                    ...
                    <button
                      ...
                      className={`p-3 rounded-xl font-bold transition ${addForm.type === 'image' ? 'bg-spectral-accent/15 text-spectral-accent-bright' : 'bg-gray-100 text-gray-600'}`}
                    >
                    ...
                    <button
                      ...
                      className={`p-3 rounded-xl font-bold transition ${addForm.type === 'api' ? 'bg-spectral-accent/15 text-spectral-accent-bright' : 'bg-gray-100 text-gray-600'}`}
                    >
            ...
            <div className="modal-footer">
              <div className="flex gap-3 w-full">
                ...
                <button
                  onClick={(event) => void runSpectralAction(event.currentTarget, handleAddCard, { action: 'add-card' })}
                  className="flex-1 ios-btn-primary px-6 py-3 rounded-xl font-bold flex items-center justify-center gap-2"
                >
