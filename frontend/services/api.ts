...
import {
  ...
  LocalLLMConfig, LocalLLMStatus, LocalLLMMessage
} from '../types';
...
export const getLocalLLMConfig = async (): Promise<LocalLLMConfig> => {
  return get('/api/local-llm/config');
};

export const updateLocalLLMConfig = async (config: LocalLLMConfig): Promise<LocalLLMConfig> => {
  return put('/api/local-llm/config', config);
};

export const getLocalLLMStatus = async (): Promise<LocalLLMStatus> => {
  return get('/api/local-llm/status');
};

export const loadLocalLLM = async (modelPath: string): Promise<Record<string, unknown>> => {
  return post('/api/local-llm/load-model', { model_path: modelPath });
};

export const unloadLocalLLM = async (): Promise<{ status: string }> => {
  return post('/api/local-llm/unload-model');
};

export const chatWithLocalLLM = async (
  messages: LocalLLMMessage[],
  config: Pick<LocalLLMConfig, 'temperature' | 'top_p' | 'max_new_tokens'>,
): Promise<{ reply: string }> => {
  return post('/api/local-llm/chat', { messages, ...config });
};

export const discoverAIModels = async (baseUrl: string, apiKey: string): Promise<string[]> => {
  const result = await post<{ success: boolean; models?: string[] }>('/ai/models', {
    base_url: baseUrl,
    api_key: apiKey,
  });
