...
注意：结合商品信息，给出实用建议。'''
    ...
    def _call_openai_api(self, client: OpenAI, settings: dict, messages: list, max_tokens: int = 100, temperature: float = 0.7) -> str:
        ...
        # DeepSeek 的推理模型会先消耗 token 进行思考；预算过小时可能以
        # finish_reason=length 结束且 content 为空。空回复不能发送给买家，
        # 因此自动用更大的预算重试一次。
        for attempt in range(2):
            request_tokens = max_tokens if attempt == 0 else max(max_tokens * 5, 500)
            try:
                ...
                logger.warning(
                    f"OpenAI API返回空内容 (attempt={attempt + 1}/2, "
                    f"finish_reason={finish_reason}, max_tokens={request_tokens})"
                )
            except Exception as e:
        ...
        raise ValueError("OpenAI API返回空回复")
...
请根据以上信息生成回复："""
                ...
                if self._is_dashscope_api(settings):
                    ...
                    # 推理模型需要额外 token 完成思考，否则可能返回空 content。
