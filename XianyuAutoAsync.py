...
class XianyuLive:
    ...
    def _extract_auto_delivery_event(self, message: dict):
        """解析闲鱼卖家端“付款完成待发货”卡片消息。

        该类推送不是普通聊天消息：付款文案位于 ``message['4']``，
        订单号嵌在 ``extJson.updateKey`` 中，不能依赖聊天消息解析流程。
        """
        if not isinstance(message, dict):
            return None

        details = message.get("4")
        if not isinstance(details, dict):
            return None

        notice = details.get("detailNotice") or details.get("reminderContent") or ""
        if not self._is_auto_delivery_trigger(str(notice)):
            return None

        reminder_url = str(details.get("reminderUrl") or "")
        item_match = re.search(r"[?&]itemId=([^&]+)", reminder_url)
        buyer_match = re.search(r"[?&]peerUserId=([^&]+)", reminder_url)
        chat_raw = message.get("2") or message.get("3") or ""
        chat_id = str(chat_raw).split("@", 1)[0]
        ...
        return {
            ...
            "buyer_id": buyer_match.group(1) if buyer_match else details.get("senderUserId"),
            "chat_id": chat_id,
            "buyer_name": details.get("reminderTitle") or "买家",
        }
            ...
            if not order_id:
                try:
                    ...
                    patterns = [
                        ...
                        # 卖家端付款卡片：updateKey=chat_id:order_id:...
                        r'updateKey"?\s*:\s*"\d+:(\d{10,}):',
                    ]
    ...
    async def _handle_auto_delivery(self, websocket, message: dict, send_user_name: str, send_user_id: str,
        ...
        try:
            ...
            from db_manager import db_manager
            if order_id in self.delivery_sent_orders or db_manager.is_order_system_shipped(order_id):
                logger.info(f'[{msg_time}] 【{self.cookie_id}】订单 {order_id} 已发送过资料，跳过重复发货')
                return
            ...
            async with order_lock:
                ...
                try:
                    ...
                    if delivery_contents:
                        ...
                        if send_failed:
                            logger.warning(f'[{msg_time}] 【自动发货】订单 {order_id} 存在发送失败，不标记已发货且不自动确认')
                            return

                        # 只有所有内容发送成功后才记录已发货，并自动确认订单。
                        self.mark_delivery_sent(order_id)
                        db_manager.insert_or_update_order(order_id=order_id, system_shipped=True, chat_id=chat_id)
                        self._lock_hold_info[lock_key] = {'locked': True, 'lock_time': time.time(), 'release_time': None, 'task': None}
    ...
    async def handle_message(self, message_data, websocket):
        ...
        try:
            ...
            # 卖家端付款完成卡片不是普通聊天消息，必须在 is_chat_message
            # 判断前单独处理，否则不会进入自动发货分支。
            payment_event = self._extract_auto_delivery_event(message)
            if payment_event:
                event_item_id = payment_event.get("item_id") or item_id
                event_buyer_id = payment_event.get("buyer_id") or user_id
                event_chat_id = payment_event.get("chat_id") or str(message.get("2") or "").split("@", 1)[0]
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】检测到卖家端付款完成卡片，"
                    f"订单={payment_event.get('order_id')}，商品={event_item_id}，聊天={event_chat_id}"
                )
                await self._handle_auto_delivery(
                    websocket,
                    message,
                    payment_event.get("buyer_name") or "买家",
                    event_buyer_id,
                    event_item_id,
                    event_chat_id,
                    msg_time,
