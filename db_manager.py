...
class DBManager:
    ...
    def init_db(self):
        ...
        try:
            ...
            # 检查并添加 system_shipped 列（用于自动发货幂等与持久化）
            try:
            ...
            # 兼容早期数据库：业务代码已按 user_id 隔离规则，但旧表没有该列，
            # 会导致新增、查询自动发货规则全部失败。
            try:
                ...
                self._execute_sql(
                    cursor,
                    "ALTER TABLE delivery_rules ADD COLUMN user_id INTEGER NOT NULL DEFAULT 1"
                )
                self._execute_sql(
                    cursor,
                    """
                    ...
                    SET user_id = COALESCE(
                        (SELECT cards.user_id FROM cards WHERE cards.id = delivery_rules.card_id),
                        1
                    )
                    """
                )
...
© 2025 闲鱼自动回复系统"""
        ...
        try:
            with self.lock:
                ...
                if row:
                    ...
                    item_info['is_multi_qty_ship'] = bool(item_info.get('multi_quantity_delivery', False))

                    # 解析item_detail JSON
                    if item_info.get('item_detail'):
                        try:
                            item_info['item_detail_parsed'] = json.loads(item_info['item_detail'])
                            detail = item_info['item_detail_parsed']
                            if isinstance(detail, dict):
                                item_info['item_image'] = detail.get('item_image') or detail.get('image') or detail.get('image_url') or ''
                        except:
        ...
        with self.lock:
            try:
                ...
                SELECT order_id, item_id, buyer_id, spec_name, spec_value,
                       quantity, amount, order_status, cookie_id, is_bargain, created_at, updated_at, version, chat_id, system_shipped
                ...
                if row:
                    return {
                        ...
                        , 'system_shipped': bool(row[14]) if len(row) > 14 else False
                    }
                return None
            except Exception as e:
    ...
    def is_order_system_shipped(self, order_id: str) -> bool:
        """返回订单是否已经发送过自动发货内容。"""
        with self.lock:
            try:
                row = self.conn.execute(
                    "SELECT system_shipped FROM orders WHERE order_id = ?", (order_id,)
                ).fetchone()
                return bool(row and row[0])
            except Exception as e:
                logger.error(f"查询订单发货标记失败: {order_id} - {e}")
                return False
